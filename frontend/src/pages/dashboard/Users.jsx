...
import {
  ...
  DropdownMenuTrigger,
  DropdownMenuPortal,
} from "../../components/ui/dropdown-menu"; // Use your existing dropdown component
...
const RoleBadge = ({ role }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-full border font-semibold ${cfg.color}`}
    >
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          disabled={saving}
          className="flex items-center gap-1.5 group outline-none focus:outline-none"
        >
          <RoleBadge role={currentRole} />
      ...
      {/* This Portal is the magic part that solves your scrolling/clipping issue */}
      <DropdownMenuPortal>
        <DropdownMenuContent 
          align="start" 
          sideOffset={8}
          ...
          {Object.entries(ROLES).map(([role, cfg]) => {
            ...
            return (
              <DropdownMenuItem
                ...
                className={`w-full flex items-start gap-3 px-3 py-2.5 hover:bg-[#F9F8F5] transition-colors text-left cursor-pointer outline-none focus:bg-[#F9F8F5] ${
                  isActive ? "bg-[#F9F8F5]" : ""
                }`}
              >
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
                    isActive ? ROLES[role].color : "bg-gray-100"
                  }`}
                >
                  <Icon className="w-3 h-3" />
                </div>
    ...
    >
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          <div>
            <Label htmlFor="u-pass" className="text-sm">
              Password
            </Label>
            <Input
              id="u-pass"
          ...
          <div>
            <Label htmlFor="u-role" className="text-sm">
              Role
            </Label>
            <select
              id="u-role"
          ...
          {/* Role preview */}
          <div
            className={`p-3 rounded-lg border text-sm ${ROLES[form.role].color}`}
          >
...
const Users = () => {
  ...
  const [filter, setFilter] = useState("all"); // all | admin | shopowner | viewer
  ...
  const avatarColor = (role) =>
    ({
      ...
      shopowner: "bg-[#C5A059]/15 text-[#C5A059]",
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          ...
          <button
            ...
            className={`bg-white border border-[#F2F0EB] border-l-4 ${color} rounded-xl p-4 text-left transition-all hover:shadow-sm ${
              filter === key ? "ring-2 ring-[#C5A059]/30 shadow-sm" : ""
            }`}
          >
            <p className="text-2xl font-bold text-[#2C2C2C]">{counts[key]}</p>
                  ...
                  >
                    ...
                    <TableCell className="pl-5">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${avatarColor(u.role)}`}
                        >
                          {getInitials(u.name)}
                        </div>
                        <div>
