...
#     current_user: User = Depends(require_role(["admin", "shopowner"])),
#     ):
#     # 1. Look up the existing category in the database
#     doc = await _db_find_one(CATEGORIES_FILE, {"id": category_id})
#     if not doc:
#         raise HTTPException(404, "Category not found")

#     # 2. CRITICAL FIX: Extract updates but FILTER OUT None values and empty strings.
#     # This prevents the "Save" button from clearing out an image you previously uploaded.
#     updates = {
#         k: v for k, v in payload.model_dump().items() 
#         if v is not None and v != ""
#     }
    
#     # 3. Handle slug generation if the name was changed
#     if "name" in updates and "slug" not in updates:
#         updates["slug"] = make_slug(updates["name"])
    
#     # 4. Perform the update only with the filtered fields
#     updated = await _db_update_one(CATEGORIES_FILE, {"id": category_id}, updates)
    
#     # 5. Log the changes for the admin audit trail
#     await log_audit(
#         current_user.id, 
#         "update", 
#         "category", 
...
# @api_router.delete("/categories/{category_id}/image")
...
async def upload_category_image(
    ...
    if not updated:
        raise HTTPException(500, "Failed to update category image")
    await log_audit(
        current_user.id,
        "update",
        "category",
